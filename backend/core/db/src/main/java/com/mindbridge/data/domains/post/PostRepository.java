...
public interface PostRepository extends JpaRepository<Post, UUID>, JpaSpecificationExecutor<Post> {
	...
	@Query(value = "SELECT p.* " + "    FROM Posts p " + "        INNER JOIN Post2tag tg "
			+ "            ON p.id = tg.post_id " + "        INNER JOIN Tags t " + "            ON tg.tag_id = t.id "
			+ "    WHERE t.name in (:tags) and p.id != :id" + "    GROUP BY p.id "
